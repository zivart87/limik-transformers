...
#### Двухколоночные секции вне контейнера (фото | текст)
Если секция — грид на весь экран (фото слева, текст справа), фиксированный `padding-right` на тексте не работает — на широких экранах текст выходит за границу контейнера.

**Правильный паттерн:**
```css
.section-text-col {
  /* правый край динамически прижимается к границе .container на любом экране */
  padding: 80px max(40px, calc((100vw - 1360px) / 2 + 40px)) 80px 60px;
}
```
Формула `(100vw - 1360px) / 2 + 40px` = авто-отступ контейнера + его padding. На экранах ≤1440px минимум 40px.

**Для левой текстовой колонки** (текст слева, фото справа) — зеркально:
```css
padding: 80px 60px 80px max(40px, calc((100vw - 1360px) / 2 + 40px));
```
