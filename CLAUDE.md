...
**Если в Heroicons нет подходящей по смыслу иконки** — искать на Flaticon (есть доступ через подключённый MCP). Выбирать вариант в том же стиле: solid-заливка одним цветом, острые углы, без обводки — под остальные правила этого раздела всё равно нужно подгонять (`fill="currentColor"`, цвет `var(--blue)`, нужный размер 64×64 или 28×28).
...
Перед добавлением нового фото на страницу — сначала проверить `assets/images/` на уже существующее нишевое фото (например `ind-utilities.webp`, `substation.webp`). Если подходящего нет — генерировать через Magnific (есть доступ через подключённый MCP), не искать сток-фото на стороне.
